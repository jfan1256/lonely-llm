...
class BertLonely(nn.Module):
    def __init__(self,
        ...
        # Multilayer Perceptron for task
        if self.configs['num_class'] == 2:
    ...
    def forward(self, index, narrative, label, reason, sentiment, device):
        ...
        if 'loss_ce' in self.configs['loss']:
            loss_ce_task = ce_loss(logits_task, label, num_class=self.configs['num_class'])
        ...
        if 'loss_center' in self.configs['loss']:
            loss_center_task = center_loss(enc_cls_output, label, self.centers, alpha=self.configs['alpha_center'])
            loss_center_sentiment = center_loss(enc_cls_output, sentiment, self.centers, alpha=self.configs['alpha_center'])
            loss_center = self.configs['alpha'] * loss_center_task + (1 - self.configs['alpha']) * loss_center_sentiment
        else:
            loss_center = torch.zeros(1, device=device)

        # ******************Angular Loss***********************
        if 'loss_angular' in self.configs['loss']:
            loss_angular_task = large_margin_cosine_loss(enc_cls_output, label, margin=self.configs['margin_angular'])
            loss_angular_sentiment = large_margin_cosine_loss(enc_cls_output, sentiment, margin=self.configs['margin_angular'])
            loss_angular = self.configs['alpha'] * loss_angular_task + (1 - self.configs['alpha']) * loss_angular_sentiment
        else:
